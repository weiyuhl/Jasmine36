 ...
 *
 * 这是一个薄薄的委托层，所有调用转发给 ConfigRepository �?ProviderRegistry�? * 统一使用 AppConfig 提供�?EncryptedConfigRepository，确保敏感数据加密存储�? */
object ProviderManager {
    ...
    fun getRagEmbeddingUseLocal(context: Context): Boolean = configRepo.getRagEmbeddingUseLocal()
    fun setRagEmbeddingUseLocal(context: Context, useLocal: Boolean) = configRepo.setRagEmbeddingUseLocal(useLocal)
    fun getRagEmbeddingModelPath(context: Context): String = configRepo.getRagEmbeddingModelPath()
    fun setRagEmbeddingModelPath(context: Context, path: String) = configRepo.setRagEmbeddingModelPath(path)
