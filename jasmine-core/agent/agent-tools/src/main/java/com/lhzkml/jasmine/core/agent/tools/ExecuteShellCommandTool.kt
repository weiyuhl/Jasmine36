...
class ExecuteShellCommandTool(
    ...
    override val descriptor = ToolDescriptor(
        ...
        description = "Executes a shell command with optional working directory and timeout. " +
            ...
            "Commands that don't complete within timeoutSeconds are moved to background — " +
            "the command keeps running and partial output collected so far is returned. " +
            "Set background=true to immediately run in background mode (useful for dev servers or long-running processes).",
