...
class AgentPromptContextProvider(
    ...
    override fun getContextSection(): String = buildString {
        ...
        appendLine("- 'general': all tools. For complex multi-step tasks.")
        appendLine("- 'explore': read-only (read_file, list_directory, find_files, search_by_regex). Fast for codebase exploration.")
        appendLine("- 'shell': shell command only. For git operations, builds, terminal tasks.")
        appendLine("- 'web': web tools only. For research and documentation.")
        appendLine()
        ...
        appendLine("- Specify exactly what information the sub-agent should return.")
        appendLine("- Launch multiple sub-agents in parallel when exploring different areas.")
