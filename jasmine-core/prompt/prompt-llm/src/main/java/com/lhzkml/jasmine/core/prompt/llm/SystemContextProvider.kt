...
/**
 * Termux 环境上下文 — 当 Termux 已安装时，注入环境信息到系统提示词
 *
 * 让模型自动知道有完整 Linux 环境可用，无需手动指定 useTermux=true。
 * 模型在执行 shell 命令时会根据命令类型自动判断是否使用 Termux 环境。
 *
 * @param termuxEnvironmentProvider Termux 环境提供者（延迟初始化）
 */
class TermuxEnvironmentContextProvider(
    private val termuxEnvironmentProvider: () -> com.lhzkml.jasmine.core.termux.TermuxEnvironment?
) : SystemContextProvider {
    override val name = "termux_environment"
    override suspend fun getContextSection(query: String?): String? {
        val termux = termuxEnvironmentProvider() ?: return null
        if (!termux.isInstalled) return null
        
        val installedPackages = try {
            termux.getInstalledPackages()
        } catch (_: Exception) {
            emptyList()
        }
        
        return buildString {
            appendLine("<termux_environment>")
            appendLine("This device has a full Termux Linux environment installed (no root required).")
            appendLine("Termux includes apt/dpkg package manager and can install python3, gcc, git, nodejs, curl, etc.")
            appendLine()
            appendLine("IMPORTANT: When executing shell commands, you MUST set useTermux=true for ALL commands that:")
            appendLine("- Need Linux tools not available on Android (python3, gcc, git, node, pip, npm, make, etc.)")
            appendLine("- Need package management (apt install/remove/update)")
            ...
            appendLine()
            appendLine("Only use useTermux=false (default) for Android-specific commands like:")
            ...
            appendLine()
            appendLine("The workspace can be accessed from Termux by using absolute paths.")
            if (installedPackages.isNotEmpty()) {
                appendLine()
                appendLine("Currently installed packages: ${installedPackages.take(20).joinToString(", ")}")
            }
            appendLine("</termux_environment>")
        }.trimEnd()
    }
